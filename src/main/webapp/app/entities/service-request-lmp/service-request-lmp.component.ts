...
@Component({
    selector: 'jhi-service-request-lmp',
    templateUrl: './service-request-lmp.component.html'
})
export class ServiceRequestLmpComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/service-request-lmp'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/service-request-lmp', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: ServiceRequestLmp) {
        return item.id;
    }
